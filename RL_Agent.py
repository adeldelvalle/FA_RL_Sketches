...
class Autofeature_agent(object):
    ...
    def __init__(self, env, bdqn_csv, learning_rate=0.05, reward_decay=0.9, e_greedy=1, update_freq=50, mem_cap=100000,
        ...
        self.eval_net = Net(self.env.get_state_len(), self.env.get_table_action_len(),
                            self.env.get_feature_action_len()).to(self.device)
        self.target_net = Net(self.env.get_state_len(), self.env.get_table_action_len(),
                              self.env.get_feature_action_len()).to(self.device)
    ...
    def choose_action(self):
        ...
        if np.random.uniform() > self.epsilon:
            ...
            if action_type == 0:
                ...
                action_num = self.env.action_feature_valid[np.random.randint(
                    self.env.get_valid_feature_action_len())] + self.env.get_table_action_len()
